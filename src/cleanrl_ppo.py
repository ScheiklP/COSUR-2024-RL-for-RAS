...
def make_env(idx, capture_video, run_name, gamma):
    def thunk():
        ...
        env = DVRKEnv(**env_kwargs)

        if capture_video and idx == 0:
            env = gym.wrappers.RecordVideo(
                env=env,
                video_folder=f"videos/{run_name}",
                step_trigger=lambda step: step == 0 or step % 10_000 == 0,
                video_length=1000,
            )

        env = gym.wrappers.TimeLimit(env, max_episode_steps=time_limit / target_dt)
...
if __name__ == "__main__":
    ...
    run_name = f"PointReach__{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
    ...
    # env setup
    envs = gym.vector.SyncVectorEnv([make_env(i, args.capture_video, run_name, args.gamma) for i in range(args.num_envs)])
    ...
    if args.save_model:
        ...
        if args.upload_model:
            ...
            repo_name = f"{args.exp_name}-seed{args.seed}"
